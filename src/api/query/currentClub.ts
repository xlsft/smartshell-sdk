...
const module = async <Response extends ResponseType>(ctx: ShellSdkContext): Promise<Response> => {
    return await ctx.request("query", "currentClub", [
        "id",
        "name",
        key("network_company", [
        ...
        key("license", [
            "id",
            "expires_at",
            key("payment", ["id", "created_at", "host_count", "payment_method", "bill_url", "months", "cost"]),
        ]),
        key("licenses", [
            "id",
            "expires_at",
        ...
        key("organization_payment_card", ["id", "card_mask", "card_payment_system", "card_expires_at", "created_at"]),
