  ...
  const color = {
    convert: {
      hex: {
        ...
        hsl: (hex)=>color.convert.rgb.hsl(color.convert.hex.rgb(hex))
      },
      rgb: {
        hex: ({ r, g, b })=>{
          try {
      ...
      hsl: {
        ...
        hex: ({ h, s, l })=>color.convert.rgb.hex(color.convert.hsl.rgb({
            h,
            s,
            l
          }))
    ...
    param: {
      luminance: (hex)=>{
        try {
          ...
          const calc = (value)=>value <= 0.03928 ? value / 12.92 : Math.pow((value + 0.055) / 1.055, 2.4);
      ...
      contrast: (f, b)=>{
        try {
          const [lf, lb] = [
            color.param.luminance(f),
            color.param.luminance(b)
          ];
